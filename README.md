...
> 개(犬) + 성(性). 성격이라는 뜻 그대로이면서, 글자 그대로는 "개의 성질"입니다.
> 저장소 슬러그와 `localStorage` 키(`dogtype:*`)는 `dogtype`으로 남아 있습니다 —
> 이름이 바뀌었다고 참가자가 저장해둔 응답과 출입증을 날릴 수는 없습니다.

60문항에 답하면 네 마리 강아지 중 하나로 판정하고, **같은 스페이스 참가자들의 관계도를
실시간으로 대형 화면에 그려주는** 웹앱입니다.

**스페이스**는 한 번의 세미나·모임 단위입니다. 누구나 이름·대표 강아지 아이콘·비밀번호로 만들 수 있고,
홈에서는 최근 24시간 안에 참여 결과가 올라온 스페이스를 찾아볼 수 있습니다. 초대 링크를 받은 사람은
비밀번호 없이 바로 참여합니다.

**함께보기**는 여러 스페이스의 결과를 데이터를 옮기지 않고 한 지도에 얹는 기능입니다. 지도 맨
아래 `공유하기`에서 상대를 고르면 제안이 가고, 상대가 수락하면 **서로** 보게 됩니다. 양쪽 다
언제든 끊을 수 있고, 연쇄되지 않습니다 — A·B가 공유하고 B·C가 공유해도 A는 C를 못 봅니다.

넘어가는 건 지도에 뜨는 것뿐입니다. 공유했다고 상대가 우리 설문에 참가하거나 발자국을 찍을
수는 없습니다 — 그러려면 우리 초대 링크나 비밀번호가 있어야 합니다.

| | |
|---|---|
| 홈 | `/` — 활성 스페이스 이름·아이콘·참여자 수·최근 활동 (입장 코드·새 스페이스는 우측 상단 메뉴) |
| 만들기 | `/new` — 이름 + 대표 강아지 아이콘 + 비밀번호, 로그인 불필요 |
...
홈 목록    /hazel-corgi-427?gate=password → 저장된 출입증이 있어도 비밀번호를 새로 확인
```
...
- 검증은 전부 Edge Function(service role)에서 합니다. `spaces` 테이블은 **RLS 정책이 하나도
  없어** anon 키로는 직접 조회할 수 없습니다. 공개 `list-active` API만 비밀번호가 설정되고
  최근 24시간 안에 활동이 있는 스페이스의 이름·코드·아이콘·참여자 수·최근 활동 시각을 내보냅니다.
  비활성 스페이스와 비밀번호 해시·토큰은 공개하지 않습니다.
- **`results`도 anon에게 닫혀 있습니다.** 읽기는 `space-views`, 쓰기는 `spaces` Edge Function만
  합니다. 둘 다 출입증(`share_token`)을 검증한 뒤에야 움직입니다 — 화면을 우회해 API를 직접
  두드려도 남의 스페이스 결과는 나오지 않습니다.
- 비밀번호 대입은 **(스페이스, IP) 당 10분에 10회**, 스페이스 생성은 **IP당 시간당 10개**,
  공유 제안은 **스페이스당 시간당 30개**로 제한됩니다. 원본 IP는 저장하지 않고 해시만 씁니다.
- **함께보기는 두 열쇠로 갈라져 있습니다.** 결과 읽기는 출입증(`share_token`), 공유를
  맺고 끊는 건 스페이스 비밀번호. 출입증으로는 공유를 만들 수도 수락할 수도 없습니다.
- **공유는 지도만 넘깁니다.** 상대가 우리 설문에 참가하거나 결과를 제출할 수는 없습니다 —
  결과를 쓰는 유일한 문은 그 스페이스의 출입증만 엽니다.
  자세한 계약은 [`docs/space-together-view-implementation-plan.md`](docs/space-together-view-implementation-plan.md) §18.

지키지 않습니다 — **알고 쓰세요**:

- **스페이스 비밀번호가 곧 관리 권한입니다.** 비밀번호를 아는 참가자는 누구나 이 스페이스의
  결과를 다른 스페이스에 공유할 수 있습니다. 초대 링크로 들어온 사람은 비밀번호를 모르니
  못 하지만, 생성자 전용 권한은 아닙니다. 참가자에게는 **초대 링크**를 주세요.
- **공유는 소급됩니다.** 공유를 맺으면 그 전에 제출된 결과까지 전부 상대에게 보입니다.
  그 사람들은 낼 때 이 공유를 몰랐습니다 — 그래서 참가 화면이 공유가 있든 없든 모두에게
  "나중에 공유되면 지금 낸 결과도 보인다"고 미리 알립니다. 그 문구를 지우지 마세요.
  (수락 이후 결과만 넘기는 게이트가 있었지만, 기존 스페이스를 연결하면 지도가 텅 비어
  걷어냈습니다. 계획 문서 §18.8.)
- **공유 대상 목록은 비밀번호가 설정된 모든 스페이스를 보여줍니다.** 홈에 안 뜨던 조용한
  스페이스도 이름과 입장 코드가 실립니다. 비밀번호를 확인한 사람에게만 보이지만, 그 사람이
  곧 참가자 전원일 수 있습니다.
- **브라우저에 도착한 결과는 캡처·복사를 막을 수 없습니다.** 함께보기의 `보기 전용`은 고치거나
  지우거나 다시 공유할 권한이 없다는 뜻이지, 복제 방지가 아닙니다.
- 홈의 `참여자 수`는 현재 접속자나 온라인 상태가 아니라 그 스페이스의 누적 참여자 수입니다.
  목록에 뜨는 기준(최근 24시간)과 인원수는 다른 이야기입니다.
...
- **참가자는 자기 결과를 지울 수 없습니다.** 결과는 스페이스가 지워질 때까지 남습니다.
  본인 삭제·재제출은 아직 없습니다.
...
- `public.spaces` 테이블 (`icon_id`, `password_hash`, `share_token`, 전역 unique 이름 포함) — **RLS 정책 없음 = anon 전면 거부**.
  service role을 쓰는 Edge Function만 읽고 씁니다. 비밀 컬럼은 컬럼 단위 GRANT로도 한 번 더 막습니다.
- `public.results` 테이블 + 스페이스 외래키 + 인덱스 + 스페이스 범위 unique 닉네임 —
  **RLS 정책 없음 + anon GRANT 없음 = 전면 거부**. 읽기·쓰기 모두 Edge Function만 합니다.
- `public.space_shares` — 함께보기 원장. **양방향**이라 사전순 정렬쌍(`space_a < space_b`)으로
  저장하고, 그 쌍이 기본키라 두 스페이스 사이의 공유는 구조적으로 하나뿐입니다.
  **RLS 정책 없음 = anon 전면 거부**. 어느 스페이스가 어디와 이어졌는지도 새 나가지 않습니다.
- `public.space_attempts` + `note_space_attempt()` — 비밀번호·관리 토큰 대입, 생성·제안 남용 제한 (service role 전용)
- `list_active_spaces()` — 비밀번호가 있고 최근 24시간 안에 활동한 스페이스 집계 (service role 전용)
- `spaces` Edge Function: 활성 스페이스 공개 목록 + 이름 중복 확인 + 스페이스 생성 + 입장 +
  **결과 저장·닉네임 확인**(출입증 검증)
- `space-views` Edge Function: **함께보기 권한과 결과 조회**. 대상 목록·제안·수락·해제(스페이스
  비밀번호) + 지도 데이터(출입증)
- `admin-spaces` Edge Function: 관리자 비밀번호 검증 + 목록·이름 수정·삭제·참가자 데이터 조회
- 방당 200명 상한 트리거 (만료가 없으므로 평생 정원입니다)
- 매시 17분 지난 입장 시도 기록 삭제 (`pg_cron`). **결과는 지우지 않습니다.**
...
```
/new                      이름·대표 강아지 아이콘·비밀번호로 스페이스 만들기 (누구나)
/                         최근 24시간 내 참여 결과가 있는 잠긴 스페이스 목록
/hazel-corgi-427          참가자 설문 — 비밀번호를 물어봄
/hazel-corgi-427?gate=password 홈 목록/메뉴 코드 입력 — 비밀번호를 새로 확인
/hazel-corgi-427#k=<토큰> 초대 링크 — 바로 입장
/hazel-corgi-427/map      해당 스페이스 닉네임·결과 관계도
/hazel-corgi-427/map?with=a-space,c-space
                          함께보기 — 공유한 스페이스를 같은 지도에 얹는다
                          (공유를 맺고 끊는 건 지도 맨 아래 `공유하기`. 비밀번호를 묻는다)
...
60문항에 어떻게 답했는지는 서버로 가지 않습니다 — 채점 결과(닉네임·유형·점수)만 갑니다.
그 결과는 스페이스가 지워질 때까지 지도에 남고, 진행자가 다른 스페이스와 함께보기를 맺었다면
그쪽 구성원에게도 보입니다. 참가 화면이 시작 전에 어느 스페이스인지 이름을 대며 알려줍니다.
