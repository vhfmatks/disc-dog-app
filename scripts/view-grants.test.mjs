...
import {
  ...
  normalizeSourceIds, pairKey, partnerOf, shareState, visibleRows
...
const active = (one, other, requestedBy = one, acceptedAt = T0) =>
  share(one, other, requestedBy, {accepted_at: acceptedAt});
const ended = (one, other, requestedBy = one) =>
  share(one, other, requestedBy, {accepted_at: T0, revoked_at: T2});
...
test('수락하기 전에는 양쪽 다 아무것도 못 본다', () => {
  ...
  assert.deepEqual(visibleRows([row('r1', 'a-space')], 'b-space', shares), []);
});
...
test('공유가 끝나면 양쪽 다 다음 조회부터 빠진다', () => {
  ...
  assert.deepEqual(deniedSourceIds(['b-space'], shares, 'a-space'), ['b-space']);
  assert.deepEqual(
    visibleRows([row('r1', 'a-space'), row('r2', 'c-space')], 'b-space', shares)
      .map(item => item.id),
    ['r2']
  );
});
...
test('남의 공유가 섞여 들어와도 내 권한으로 읽지 않는다', () => {
  ...
  assert.deepEqual(visibleRows([row('r1', 'b-space')], 'a-space', others), []);
});
...
test('공유하면 언제 제출됐든 전부 넘어간다 — 소급이다', () => {
  // 예전에는 수락 시각(visible_from) 이후 결과만 넘겼다. 그러면 기존 스페이스 둘을
  // 연결했을 때 지도가 텅 비어 기능이 쓸모없었다. 지금은 제출 시각을 보지 않는다.
  //
  // ⚠ 그 대가가 소급 노출이다: 공유 전에 제출한 사람들도 상대에게 보인다. 그 사람들은
  //   낼 때 이 공유를 몰랐다. 참가 화면의 사전 고지가 그 짝이다 (아래 테스트).
  const shares = [active('a-space', 'b-space', 'a-space', T1)];   // T1에 수락
  const rows = [
    row('before', 'a-space', T0),   // 수락 전에 제출 — 이제 이것도 보인다
    row('exact', 'a-space', T1),
    row('after', 'a-space', T2)
  ];

  assert.deepEqual(
    visibleRows(rows, 'b-space', shares).map(item => item.id),
    ['before', 'exact', 'after']
  );
  ...
  assert.deepEqual(
    visibleRows(mine, 'a-space', shares).map(item => item.id),
    ['mine-before', 'mine-after']
  );
});

test('해제했다 다시 맺으면 그동안의 결과까지 전부 다시 보인다', () => {
  // 게이트가 없으니 재수락에 되돌리는 힘도 없다. 해제는 "지금부터 안 보임"일 뿐,
  // 한 번 본 것을 없던 일로 만들지 못한다 — 애초에 브라우저가 이미 받아갔다.
  const rows = [row('old', 'a-space', T0), row('new', 'a-space', T2)];

  assert.deepEqual(visibleRows(rows, 'b-space', [ended('a-space', 'b-space')]), []);
  assert.deepEqual(
    visibleRows(rows, 'b-space', [active('a-space', 'b-space', 'a-space', T2)]).map(r => r.id),
    ['old', 'new']
  );
});

test('공유가 하나도 없으면 내 스페이스 결과만 본다', () => {
  const rows = [row('mine', 'b-space', T0), row('theirs', 'a-space', T0)];

  assert.deepEqual(visibleRows(rows, 'b-space', []).map(item => item.id), ['mine']);
});

test('공유가 없는 스페이스의 행은 섞여 들어와도 걸러진다', () => {
  // fail closed. 위 단계에서 이미 막지만, 마지막 필터도 혼자서 옳아야 한다.
  assert.deepEqual(visibleRows([row('r1', 'd-space')], 'b-space', []), []);
});
...
test('visible_from 게이트는 컬럼까지 통째로 사라졌다', async () => {
  // 반쯤 남으면 읽는 사람이 "이게 뭘 막고 있지?"를 묻게 된다. 죽은 컬럼이 더 나쁘다.
  const [nine, schema, views, shared] = await Promise.all([
    readFile(new URL('../prisma/migrations/9_share_all_results/migration.sql', import.meta.url), 'utf8'),
    readFile(new URL('../prisma/schema.prisma', import.meta.url), 'utf8'),
    readFile(new URL('../supabase/functions/space-views/index.ts', import.meta.url), 'utf8'),
    readFile(new URL('../supabase/functions/_shared/view-grants.ts', import.meta.url), 'utf8')
  ]);

  assert.match(nine, /alter table public\.space_shares drop column if exists visible_from/);
  assert.match(nine, /drop constraint if exists space_shares_visible_needs_accept/);
  assert.doesNotMatch(schema, /visible_from/);
  // 주석에서 "예전엔 이랬다"고 설명하는 건 괜찮지만, 코드가 읽거나 쓰면 안 된다.
  assert.doesNotMatch(views, /visible_from:/);
  assert.doesNotMatch(shared, /share\.visible_from/);
});

test('소급 노출의 짝 — 참가 화면이 공유 전에 미리 고지한다', async () => {
  // ⚠ 이 문구가 이 결정의 전부다. 공유가 소급되므로, 지금 공유가 없는 스페이스의
  //   참가자도 "나중에 맺으면 오늘 낸 것도 보인다"를 알아야 한다. 그래서 고지는
  //   공유 유무와 무관하게 언제나 나온다.
  const participant = await readFile(new URL('../src/ParticipantApp.tsx', import.meta.url), 'utf8');

  // 공유가 걸린 스페이스: 이름을 대고, 기존 결과도 포함임을 밝힌다.
  assert.match(participant, /이미 제출된 결과도 함께/);
  // 긴 설명은 기본으로 접어 두되 제목에서 열어 볼 수 있어야 한다.
  assert.match(
    participant,
    /<details className="callout shared-notice"[^>]*>\s*<summary className="shared-notice-summary">/
  );
  assert.doesNotMatch(participant, /<details className="callout shared-notice"[^>]*\sopen(?:=|\s|>)/);
  // 공유가 없는 스페이스: 나중에 맺힐 수 있음을 조용히 알린다.
  assert.match(participant, /진행자가 나중에 다른 스페이스와[\s\S]{0,40}맺으면/);
  // "지금부터 제출되는" 은 이제 거짓말이다.
  assert.doesNotMatch(participant, /지금부터 제출되는/);
});

test('공유 화면이 맞교환과 소급을 둘 다 말한다', async () => {
  // 버튼을 누르는 사람이 둘 중 하나라도 모르면 남의 데이터를 잘못 내보내는 것이다.
  const share = await readFile(new URL('../src/components/ShareSpaces.tsx', import.meta.url), 'utf8');

  assert.match(share, /지금까지 제출된 결과도 전부/);     // 수락 버튼 옆
  assert.match(share, /양쪽의 기존 참가자까지[\s\S]{0,20}전부/); // 공유 목록
  assert.doesNotMatch(share, /수락 이후에 제출되는 결과부터/);
});
